...
if (process.argv[2] === "--enable-debug") {
    console.log("[*] Kernel Debug Messaging Enabled....");
    debug = true;
}
        ...
        try {
            ...
            switch (NodeInfo.type) {
                case "PING":
                    ...
                    ev.on("close", () => {
                        for (let id in NodeConnectionRegister) {
                            if (NodeConnectionRegister[id] === NODE_ID) {
                                ...
                                console.log("[GPU Node: -> ID [ " + NODE_ID + " ]::[DISCONNECTED]");
                            }
                        }
                    ...
                    console.log("[GPU Node: -> ID [ " + NodeInfo.nodeID + " ]::[CONNECTED]");
                    break;
                ...
                case "DATA":
                    if (ActiveNodes.hasOwnProperty(NodeInfo.nodeID)) {
                        ...
                        if (Object.keys(NodeInfo.KERNEL_DATA).length > 0) {
                            NodeData[NodeInfo.nodeID] = NodeInfo.KERNEL_DATA;
                            console.log("[+] GPU_NODE [ID]::" + NodeInfo.nodeID + " [DATA_DUMP]::[OK]");
                        } else {
                ...
                case "KERNEL":
                    let index = 0;
                    let dataOffset = 0;
                    let endOffset = NodeInfo.kernelCount - 1;
                    if (NodeInfo.kernelCount != 0) {
                        for (let index in ActiveNodes) {
                            ...
                            dataOffset += NodeInfo.kernelCount;
                            endOffset += NodeInfo.kernelCount;
                            console.log("[*] Deploying on [GPU Node]::" + (index));
                            index++;
                        }

                    } else {
                        console.log("[x] Invalid Kernel Count :: [ERR_KERN_COUNT]");
                    }
                    break;
